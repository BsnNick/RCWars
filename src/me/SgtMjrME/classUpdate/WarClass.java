...
public class WarClass {
	...
	public WarClass(String string, RCWars rcWars) {
	    System.out.println("Creating WarClass: " + string);
		pl = rcWars;
	...
	public static void loadClasses(RCWars rcWars) {
	    System.out.println("WarClass.loadClasses()...");
		File f = new File(rcWars.getDataFolder().getAbsolutePath() + File.separator + "Classes");
		if (!f.exists())
			f.mkdirs();
		for (String file : f.list())
			if (file.endsWith(".yml")) {
			    System.out.println("Loading WarClass" + file);
				WarClass c = new WarClass(f.getAbsoluteFile() + File.separator + file,
						rcWars);
				if (!c.valid) {
